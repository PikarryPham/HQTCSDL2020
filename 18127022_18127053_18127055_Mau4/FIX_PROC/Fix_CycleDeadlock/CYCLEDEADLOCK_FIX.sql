...
BEGIN TRANSACTION
	BEGIN TRY
		...
		BEGIN
			...
			set @RETURNCODE = 1;
			set @RETURNMESS = N'Không thể cập nhật lương hoặc xóa chính bản thân nhân viên quản lý'
		...
		BEGIN
			PRINT N'Không được cập nhật lương của nhân viên quản lý khác';
			set @RETURNCODE = 1;
			set @RETURNMESS = N'Không được cập nhật lương của nhân viên quản lý khác';
			ROLLBACK TRANSACTION
			RETURN;
		END
		...
		set @RETURNMESS = N'Danh sách nhân viên (tính cả nhân viên quản lý) làm việc tại' + @CHINHANHNVQL + N' khi chưa cập nhật lương'
		...
		BEGIN
			...
			BEGIN
				...
				set @RETURNCODE = 1;
				set @RETURNMESS = N'Không tồn tại nhân viên này trong bảng nhân viên'
				ROLLBACK TRANSACTION
				RETURN;
			END
			ELSE 
			BEGIN 
				...
				set @RETURNMESS = N'Danh sách nhân viên (tính cả nhân viên quản lý) làm việc tại' + @CHINHANHNVQL + N' sau khi đã nhật lương'
				SELECT NHANVIEN.MA_NV, NHANVIEN.TEN_NV,
				NHANVIEN.LUONG, NHANVIEN.LAMVIEC FROM NHANVIEN WHERE NHANVIEN.LAMVIEC = @CHINHANHNVQL
			END
		END
...
BEGIN TRAN
		BEGIN TRY
			...
			BEGIN
				...
				set @RETURNCODE = 1;
				set @RETURNMESS = N'Nhân viên này không tồn tại'
				ROLLBACK TRANSACTION
				RETURN;
			END
		...
		BEGIN CATCH
				IF(ERROR_NUMBER() = 1205)
				 BEGIN
						set @RETURNCODE = 1;
						set @RETURNMESS = 'Deadlock Occurred. The Transaction has failed. Please retry'
		...
		set @RETURNCODE = 0;
		set @RETURNMESS = N'Chỉnh sửa thông tin cá nhân thành công'
COMMIT TRAN